...
export function NoticeTypesTable() {
  ...
  const { data: types = [], error, isLoading, isFetching } =
    useGetNoticeTypesQuery(undefined, {
      skip: !canFetch,
    });
  const [deleteType, deleteState] = useDeleteNoticeTypeMutation();
  ...
  const filtered = useMemo(() => {
    ...
    return types.filter((item) => item.title.toLowerCase().includes(query));
  }, [appliedSearch, types]);
  ...
  async function confirmDelete() {
    ...
    try {
      ...
      setDeleteError(getApiErrorMessage(caught, "Could not delete notice type"));
    }
  }
      ...
      <article className="overflow-hidden rounded-2xl bg-white shadow-[0_8px_30px_rgb(0,0,0,0.04)]">
        <div className="overflow-x-auto">
          <table className="min-w-full text-left text-sm">
            ...
            <tbody>
              {isLoading ? (
                <TableLoadingRow colSpan={3} label="Loading notice types" />
              ) : error ? (
                <tr>
                  <td
                    colSpan={3}
                    ...
                    role="alert"
                  >
                    {getApiErrorMessage(error, "Could not load notice types")}
                  </td>
                </tr>
              ) : filtered.length === 0 ? (
                <tr>
                  ...
                  >
                    No notice types found. Click Add to create one.
                  </td>
                </tr>
              ) : (
                filtered.map((item) => (
                  ...
                  >
                    ...
                    <td className="px-5 py-4">
                      <div className="flex items-center justify-end gap-2">
                        ...
                        <Link
                          href={`/notices/types/${item.id}/edit`}
                          aria-label="Edit"
                          title="Edit"
                          className="inline-flex h-11 w-11 cursor-pointer items-center justify-center rounded-xl border border-border bg-white hover:bg-primary-soft hover:text-primary"
                        >
