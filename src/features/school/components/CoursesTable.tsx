...
export function CoursesTable() {
  ...
  const { data: courses = [], error, isLoading, isFetching } = useGetCoursesQuery(
    undefined,
    { skip: !canFetch },
  );
  ...
  const filtered = useMemo(() => {
    ...
    return courses.filter((item) => item.title.toLowerCase().includes(query));
  }, [appliedSearch, courses]);
  ...
  async function confirmDelete() {
    ...
    try {
      ...
      setDeleteError(getApiErrorMessage(caught, "Could not delete course"));
    }
  }
      ...
      <article className="overflow-hidden rounded-2xl bg-white shadow-[0_8px_30px_rgb(0,0,0,0.04)]">
        <div className="overflow-x-auto">
          <table className="min-w-full text-left text-sm">
            ...
            <tbody>
              {isLoading ? (
                <TableLoadingRow colSpan={4} label="Loading courses" />
              ) : error ? (
                <tr>
                  <td
                    colSpan={4}
                    ...
                    role="alert"
                  >
                    {getApiErrorMessage(error, "Could not load courses")}
                  </td>
                </tr>
              ) : filtered.length === 0 ? (
                <tr>
                    ...
                    <td className="px-5 py-4">
                      <div className="flex items-center justify-end gap-2">
                        ...
                        <button
                          ...
                          aria-label="Delete"
                          title={
                            item.classCourseCount > 0
                              ? "Assigned to classes — cannot delete"
                              : "Delete"
                          }
                          disabled={item.classCourseCount > 0}
                          onClick={() => {
                            setDeleteError(null);
